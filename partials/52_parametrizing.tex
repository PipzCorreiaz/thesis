...
The final points obtained by the team with one Deep-1 player and one Rule-based (f, pink) were higher than the ones obtained by the team with one Trick and one Rule-based (d, yellow), which is visible from the density between 80 and 120 points.
At the same time, the team with two Deep-1 players present a higher discrepancy from the team with two Trick players.
...
Figure~\ref{fig:DEFG-CH} presents the Gaussian fitting curves of final points achieved by
By comparing results on \emph{medium} initial conditions, from scenario (d) to
